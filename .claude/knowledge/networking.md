...
`/admin/scions/` proxies to `forge-web:8200`, which has no app-layer auth (it's designed to bind 127.0.0.1 on its host). As of **Nooscope-hm4c** the gateway gates on `$admin_valid`, a variable computed by njs (`njs/nooscope-auth.js`, loaded via `nginx.main.conf`) that returns `"1"` only when the request carries an **HMAC-signed, unexpired `nooscope_admin` session cookie**. That cookie is minted exclusively by `POST /admin/login`, whose njs handler verifies the password **server-side** (SHA-256 compared to `$admin_hash`, injected by the entrypoint and never sent to the browser) before issuing the signed, HttpOnly cookie; `POST /admin/logout` expires it. The gateway then injects the real `Authorization: Bearer $FORGE_WEB_ADMIN_TOKEN` upstream. Forging the cookie value no longer passes — a client that doesn't know the password can't produce a valid signature. The HMAC key (`$session_secret`) is a fresh random per-boot value (or `NOOSCOPE_SESSION_SECRET` to pin it). The **same `$admin_valid`** gates the per-Scion engram/forge WS routes and keys the per-Scion morpheus-bearer `map`, so every admin path shares one verified signal.
